...
bool takeDataPiHasCommand() {
  if (ensureMqttConnected()) {
    ...
    if (now - lastHeartbeatMs > HEARTBEAT_INTERVAL_MS) {
      lastHeartbeatMs = now;
      const char* statusPayload = "{\"status\": \"ONLINE\", \"device\": \"medilink-001\"}";
      mqttClient.publish(MQTT_TOPIC_COMMAND, statusPayload);
    }
  }
...
void takeDataPiPublish(const char* topic, const char* payload) {
  if (ensureMqttConnected()) {
